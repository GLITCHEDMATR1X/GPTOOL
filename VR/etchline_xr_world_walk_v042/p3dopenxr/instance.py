...
class Instance:
    ...
    def debug_callback_py(
        ...
        d = data.contents
        self.logger.log(
            py_log_level(severity),
            f"OpenXR: {_xr_text(d.function_name)}: {_xr_text(d.message)}",
        )
        return False
    ...
    def destroy(self):
        if self.handle is not None:
            try:
                ...
                self.logger.warning('OpenXR instance destroy raised: %s', exc)
            finally:
                self.handle = None
    ...
    def _log_extensions(self, layer_name, indent: int = 0):
        ...
        for extension in extension_properties:
            self.logger.debug(
                f"{indent_str}  Name={_xr_text(extension.extension_name)} SpecVersion={extension.extension_version}")
    ...
    def log_instance_info(self):
        ...
        self.logger.info(
            ...
            f"Instance RuntimeName={_xr_text(instance_properties.runtime_name)} "
    ...
    def log_layers(self):
        ...
        for layer in layers:
            self.logger.debug(
                f"  Name={_xr_text(layer.layer_name)} "
                f"LayerVersion={layer.layer_version} "
                f"Description={_xr_text(layer.description)}")
            self._log_extensions(layer_name=_xr_text(layer.layer_name), indent=4)
