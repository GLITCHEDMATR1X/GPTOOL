#!/usr/bin/env python3
"""GPTOOL AI Patch Gate pass combiner for iterative game prototype zips.

Version: GPTOOL Pass 17 AI Patch Gate drop-in. Based on Safe Pass Combiner v6.

Purpose
-------
This GPTOOL patch gate combines a base project folder/zip with a sequence of patch-only zips
...
def _friendly_no_args(tool_name: str) -> int:
    print("")
    print(f"{tool_name} needs command-line options, so double-clicking this .py file by itself cannot know what to combine.")
    print("")
    print("Use one of these instead:")
    print("  START_HERE.bat                         interactive menu; safest")
    print("  START_HERE_REPO_PATCH.bat              repo patch dry-run/apply menu")
    print("  DRY_RUN_HOLOCORE_PATCH_TO_GX_REPO.bat  quick dry-run template")
    print("")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not raw_argv:
        return _friendly_no_args("Safe Pass Combiner")
