...
def load_simulation_inputs(holoverse_root: Path | str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "population_rules": _read_json(root / "population" / "residential_population_rules.json") if (root / "population" / "residential_population_rules.json").exists() else {},
...
def simulate_city_at_time(holoverse_root: Path | str | None, clock: str, *, danger_state: bool = False, inputs: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    residential_population_model: dict[str, Any] = {}
    try:
        from holoutopia_residential_population import build_residential_population_frame

        residential_population_model = build_residential_population_frame(
            holoverse_root,
            clock,
            inputs=data,
            node_index=node_index,
            rules=data.get("population_rules") if isinstance(data.get("population_rules"), dict) else None,
        )
    except Exception as exc:
        issues.append(f"residential-population-model:{exc.__class__.__name__}:{exc}")
    ...
    return {
        ...
        "world_population": int(residential_population_model.get("world_population", len(citizens_out)) or len(citizens_out)) if residential_population_model else len(citizens_out),
        "residential_population_model": residential_population_model,
